        ...
        <a-card class="mb-4" :bordered="false">
            <a-form :model="searchForm" layout="inline" class="search-form">
                <a-form-item label="类型编号">
                    <a-input v-model:value="searchForm.typeCode" placeholder="请输入内容" style="width: 220px" />
                </a-form-item>
                <a-form-item label="类型名称">
                    <a-input v-model:value="searchForm.typeName" placeholder="请输入内容" style="width: 220px" />
                </a-form-item>
                <a-form-item>
                    <a-space>
...
// 表格数据
const tableData = ref<MaterialType[]>([]);
...
// 模拟数据
const mockData: MaterialType[] = Array.from({ length: 56 }, (_, i) => ({
    id: i + 1,
    typeCode: `LXBH${String(i + 1).padStart(9, '0')}`,
    typeName: ['原材料', '半成品', '成品', '辅料', '包装材料', '备件'][i % 6],
    status: 'enabled',
    remark: '无',
...
const loadData = () => {
    ...
    if (searchForm.typeCode) {
        filteredData = filteredData.filter(item =>
            item.typeCode.toLowerCase().includes(searchForm.typeCode.toLowerCase())
        );
    }
    if (searchForm.typeName) {
        filteredData = filteredData.filter(item => item.typeName.includes(searchForm.typeName));
    }

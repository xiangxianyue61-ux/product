...
// 加载数据（从 MongoDB: create.OnTimeRate，经 hd /api/productionOnTimeRates/list）
const loadData = async () => {
    try {
        ...
        // productType 暂未入库字段，保留前端筛选可在后续扩展

        const res = await apiFetch<ListResult<OnTimeRate>>(`/api/productionOnTimeRates/list?${params.toString()}`);
        tableData.value = res.data ?? [];
        pagination.total = res.total ?? 0;

        // 计算 KPI
        kpiStats.plannedQuantity = tableData.value.reduce((sum, item) => sum + item.plannedQuantity, 0);
        kpiStats.completedQuantity = tableData.value.reduce((sum, item) => sum + item.onTimeCompletedQuantity, 0);
        kpiStats.onTimeRate =
        ...
        tableData.value = [];
        pagination.total = 0;
        message.error(`获取准时率数据失败：${(e as Error).message || '未知错误'}`);
    }
};
